...
function BetterFlags.start()
    ...
    BetterFlags.capturaDeEventoOnline()
...
function BetterFlags.shouldSlowCar()
    if (CAR.speedKmh < 30) and not(CAR.isInPitlane) and (CAR.wheelsOutside < 3) and (SIM.timeToSessionStart < -10000) then
        if lastSlowCarBroadcastAttempt + slowCarCooldown < BetterFlags.totalElapsedTime then
            ...
            BetterFlags.slowCarEvent({slowCarProgress=trackProgress})
        end
...
local eventoCarroLento
function BetterFlags.capturaDeEventoOnline()
    eventoCarroLento = ac.OnlineEvent({
        structCarroLento = ac.StructItem.key("carroLento"),
        structLocalCarroLento = ac.StructItem.float(),
