...
ac.onOnlineWelcome(function(message, config) --Reads the script config from the extra options
    ...
    configCheck = config:mapSection("BETTERFLAGS",
        { NO_OVERTAKE_ZONE_1 = { 0, 0 }, NO_OVERTAKE_ZONE_2 = { 0, 0 }, NO_OVERTAKE_ZONE_3 = { 0, 0 } })

    ac.debug("config", configCheck)

    noOvertake1_S, noOvertake1_E = config:get("BETTERFLAGS", "NO_OVERTAKE_ZONE_1", 0),
        config:get("BETTERFLAGS", "NO_OVERTAKE_ZONE_1", 0, 2)
    noOvertake2_S, noOvertake2_E = config:get("BETTERFLAGS", "NO_OVERTAKE_ZONE_2", 0),
        config:get("BETTERFLAGS", "NO_OVERTAKE_ZONE_2", 0, 2)
    noOvertake3_S, noOvertake3_E = config:get("BETTERFLAGS", "NO_OVERTAKE_ZONE_3", 0),
...
function flagHandler()
    -- Ele checa se o carro está em uma zona de no overtaking, e se estiver ele ativa a bandeira de no overtaking. Se não estiver, ele desativa a bandeira. O mesmo processo é repetido para as outras bandeiras, cada uma com sua própria condição de ativação.
...
--[[ac.onSharedEvent("broadcastSlowCar", function(slowCarProgress)
    if ((slowCarProgress+0.01) > trackProgress-math.floor((StateWhkProgress + slowCarDistance)) and (slowCarProgress < ((trackProgress + slowCarDistance)-math.floor((trackProgress + slowCarDistance))))) then
        lastSlowCarRecieve = totalElapsedTime
    end
end)]]
...
function script.drawUI() --Draws a shitty UI for it.
    ...
    --coloca as bandeiras uma do lado da outra quando mais de uma 
    flagsWindow:clear()
